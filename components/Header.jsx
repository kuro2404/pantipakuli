...
function Header() {
  ...
  useEffect(() => {
    if (Object.keys(auth).length > 0) {
      axios
        .get(`/api/user/balance?userName=${auth.user.userName}`)
        .then((response) => {
    ...
    <div className="w-full bg-black  border-white border-b-2  ">
      <div className="w-full flex justify-between ">
        <div className="bg-black   h-[70px] ">
          <h1
            className="font-bold  text-2xl font-mono  text-white"
            id="balance"
          >
            Balance: {balance}
          </h1>
        ...
        <div className="bg-black text-2xl  w-[250px] h-[70px] ">
          <h1 id="currentTime" className="text-white  font-mono">
            {" "}
            Draw: {nextToDrawtime}
          </h1>
        </div>
        ...
        <div className="flex mt-[10px]  ">
          <div className="flex space-x-5 ">
            <Link href="/rules">
              <img className=" h-[40px]" src="/question.png" />
            </Link>
            <img
              onClick={pageReload}
